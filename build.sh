...
function BuildGO() {
    CGO_ENABLED=1 CC=arm-linux-gnueabihf-gcc GOARCH=arm go build -o $BUILDROOT/target/keybase -tags production github.com/keybase/client/go/keybase
    CGO_ENABLED=1 CC=arm-linux-gnueabihf-gcc GOARCH=arm go build -o $BUILDROOT/target/kbfsfuse -tags production github.com/keybase/client/go/kbfs/kbfsfuse
    CGO_ENABLED=1 CC=arm-linux-gnueabihf-gcc GOARCH=arm go build -o $BUILDROOT/target/git-remote-keybase -tags production github.com/keybase/client/go/kbfs/kbfsgit/git-remote-keybase
}
